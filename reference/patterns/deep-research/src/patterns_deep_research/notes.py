...
"""Match the lead sentence up to and including its terminator (``.``, ``!`` or ``?``).

The terminator must be followed by whitespace (any of space/newline/tab) or the end
of the snippet, so an in-word ``.`` (e.g. an abbreviation mid-sentence) is not treated
as a boundary. ``re.DOTALL`` lets ``.`` span newlines so a sentence wrapping a line
break is still captured whole.
"""
...
def _key_point(snippet: str, limit: int) -> str:
    ...
    The lead sentence (the highest-signal part of a snippet) is extracted up to its
    first sentence terminator — one of ``.``, ``!`` or ``?`` followed by whitespace
    (space, newline, tab) or the snippet end — and that lone trailing terminator is
    stripped so the key point reads cleanly. When no terminator is found the whole
    snippet is kept (then char-capped), preserving the prior fallback behaviour.
    An empty or whitespace-only snippet yields an empty key point by design — the
    note still carries its ``(source, locator, score)`` anchor for dedup/ranking,
    and downstream eval maps an empty key point to ``Unknown`` (insufficient
    evidence; spec 011 Req 2.3).
    """
    stripped = snippet.strip()
    match = _FIRST_SENTENCE.match(stripped)
    head = (match.group() if match else stripped).strip().rstrip(".!?")
    if len(head) <= limit:
        return head
